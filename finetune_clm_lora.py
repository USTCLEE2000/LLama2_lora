...
# BertConfig: BertForCausalLM, GPT2Config -> GPT2LMHeadModel,获取所有的配置类,并作为ModelArguments的参数model_type使用,来指定要使用的模型类型。
MODEL_CONFIG_CLASSES = list(MODEL_FOR_CAUSAL_LM_MAPPING.keys())
MODEL_TYPES = tuple(conf.model_type for conf in MODEL_CONFIG_CLASSES)


# 定义模型参数、数据参数和训练参数等配置类。
@dataclass
    ...
    def on_save(
        ...
        **kwargs,
        ...
        if state.is_world_process_zero:
            ...
            # 使用model.save_pretrained()来保存模型。默认Trainer在保存模型时,使用model.save(),它不会保存自定义模型如Lora的状态信息
...
def main():
    ...
    # 自动实现了checkpoint的检测和加载,方便模型训练过程中的断点续训
    ...
    if True:
        ...
        # load_dataset会根据extension自动判别格式,加载csv或txt
    ...
    # target_modules给了我们灵活指定模型中要应用Lora的模块的方式。只对关键的模块应用Lora可以减少计算量
    ...
    # 获取输入数据的列名,根据列数判断是否是单文本输入或文本对输入
    ...
    # 使用main_process_first加速，分词
    with training_args.main_process_first(desc="dataset map tokenization"):
        # 默认为False,整个数据集会先完全加载到内存,然后进行预处理等操作
    ...
    # 将预训练模型转换为支持Lora的模型,使用get_peft_model函数
    ...
    trainer = Trainer(
        ...
        # 将文本转化为token id,并加上特殊tokens如BOS/EOS；打包成统一的batch
    ...
    if training_args.do_train:
        ...
        if training_args.resume_from_checkpoint is not None:
            # 如果指定了resume_from_checkpoint,则加载该checkpoint
            ...
            # checkpoint = Fa
        # 否则,如果有最后一次的checkpoint,加载它到checkpoint变量
